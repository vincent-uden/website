...
export default function Menu() {
  ...
  return (
    <>
      <div
        class={`w-screen lg:w-[50rem] h-screen bg-pale-grey fixed top-0 left-0 transition-transform shadow-menu ${
          open()
            ? "translate-x-0"
            : "-translate-x-[100vw] lg:-translate-x-[calc(50rem-15rem)]"
        }`}
      >
        <div class="h-32" />
        ...
        <div class="flex flex-col gap-4 lg:gap-8 pl-8 lg:pl-16">
          <a href="/">
            <p class="font-geo text-4xl lg:text-5xl text-slate-dark hover:text-slate-blue">HOME</p>
          </a>
          <a href="/blog">
            <p class="font-geo text-4xl lg:text-5xl text-slate-dark hover:text-slate-blue">BLOG ARCHIVE</p>
          </a>
        </div>
        ...
        <div class="absolute top-0 right-0 h-full w-24 bg-slate-blue flex-col items-center justify-center text-pale-grey cursor-pointer hidden lg:flex">
          <BiRegularChevronsRight
            class={`transition-transform ${open() ? "rotate-180 -translate-x-1" : "rotate-0 translate-x-1"}`}
            size={96}
            onClick={() => setOpen((x) => !x)}
          />
        </div>
